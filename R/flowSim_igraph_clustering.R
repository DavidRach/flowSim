...
#' 
#' @importFrom magrittr %>%
#' @importFrom igraph graph_from_data_frame set_vertex_attr vertex_attr
#' 
#' @return two objects
...
#' @export
#' @examples A <- 2+2
gen_igraph_network<-function(similarity_output,method_clust="louvain", seed_n=40){
  set.seed(seed_n)
  print("------- generate igraph network ----------")
  igraph_network <- graph_from_data_frame(d = similarity_output$edges,vertices=similarity_output$nodes,directed = FALSE)
  ...
  igraph_network<-igraph_network %>% set_vertex_attr(name="color",
    index = vertex_attr(igraph_network)$name,value = df_groups$final_col_vec)
...
#' @param method clustering method: louvain, infomap. Default to louvain
#' 
#' @importFrom igraph cluster_infomap cluster_louvain membership vertex_attr
#' 
#' @keywords Internal
